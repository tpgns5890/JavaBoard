		...
		while (true) {
			...
			try {
				...
				if (logMenu == 1) {
					...
					userId = scn.nextLine();
					boolean userCheck = dao.userCheck(userId);
					if (userCheck == true) {
											...
											if (boardMenu == 1) {
												...
												while (true) {
													...
													if (brd != null) {
														...
														if (repMenu == 1) {
															dao.insertRep(no, userId);
														} else if (repMenu == 2) {
															dao.deleteRep(userId);
														} else if (repMenu == 3) {
															String getMsg = brd.getWriter();
															dao.sendMsg(getMsg, userId);
														} else if (repMenu == 4) {
															break;
														}
													}
